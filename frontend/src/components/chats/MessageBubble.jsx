...
const COLLAPSE_THRESHOLD = 500; // characters before offering collapse
...
export default function MessageBubble({ role, content, createdAt, onEdit }) {
  ...
  return (
    <div className={`flex w-full flex-col ${isUser ? "items-end" : "items-start"}`}>
        ...
        {isLong && (
          <button
            ...
            className={`
              ...
              ${isUser ? "text-white/80 hover:text-white" : "text-[#ff7a18] hover:text-[#ff4d00]"}
            `}
          >
      ...
      {/* Action row: copy, edit, timestamp */}
      <div
        className={`
          mt-1 flex items-center gap-3 px-1 text-xs
          text-[#6b6b73] dark:text-[#8a8a92]
        `}
      >
